...
Высокопроизводительная система ротации Tor-прокси с веб-интерфейсом управления, автоматическим балансированием нагрузки и мониторингом в реальном времени. **Использует HTTP туннель Tor для честного round-robin балансирования вместо SOCKS5.**

## 🚀 Ключевые возможности

- **HTTP туннель Tor** вместо SOCKS5 для честной балансировки
...
## 🔄 HTTP туннель вместо SOCKS5

### Преимущества HTTP туннеля Tor

**Проблема SOCKS5 + HAProxy:**
- HAProxy работает в TCP режиме для SOCKS5
- Не может понять границы запросов
- Round-robin работает на уровне TCP соединений, а не HTTP запросов
- Один клиент может "застрять" на одном Tor узле

**Решение HTTP туннель:**
- HAProxy работает в HTTP режиме  
- Понимает границы каждого HTTP запроса
- Честный round-robin на уровне запросов
- Каждый запрос может попасть на разный Tor узел
- Лучшая изоляция и распределение нагрузки

### Как это работает

```
HTTP Client → HAProxy (HTTP mode) → Tor HTTP Tunnel → Internet
     ↓              ↓                      ↓
  Request 1    →  Tor Node 1         →  Exit IP 1
  Request 2    →  Tor Node 2         →  Exit IP 2  
  Request 3    →  Tor Node 3         →  Exit IP 3
  Request 4    →  Tor Node 1         →  Exit IP 1
```
