...
from models import (
    SubnetData, Stats, ServiceStatus,
...
class TorNetworkManager:
            ...
            if 'or_addresses' in relay:
                for addr in relay['or_addresses']:
                    ...
                    if ':' not in ip:
                        relays.append({
                            'ip': ip,
                            'country': relay.get('country', 'Unknown'),                            'as_name': relay.get('as_name', 'Unknown')
                        })
        return relays
    ...
    def emit_subnet_data(self, relays):
        ...
        for subnet, count in sorted_subnets:
            ...
            instance_count = len(self.subnet_tor_processes.get(subnet_key, {}))
