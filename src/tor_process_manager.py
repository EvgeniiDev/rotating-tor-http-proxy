...
class TorProcessManager:
    ...
    def _start_instance(self, subnet):
        ...
        tor_config_result = self.config_manager.create_tor_config_by_port(port, subnet)
        tor_cmd = ['tor', '-f', tor_config_result['config_path']]

        logger.info(f"Starting Tor instance on port {port} for subnet {subnet}")
    ...
    def start_subnet_instances(self, subnet, instances_count=1):
        ...
        logger.info(f"Started {len(started_ports)} Tor instances for subnet {subnet}")
    ...
    def _terminate_process(self, process, port, subnet):
        if process and process.poll() is None:
            process.terminate()
            logger.info(f"Stopped Tor instance on port {port} for subnet {subnet}")
    
    def restart_instance_by_port(self, port, subnet):
        logger.info(f"Restarting Tor instance on port {port} for subnet {subnet}")
        ...
        with self._lock:
            ...
            if new_process:
                ...
                logger.info(f"Successfully restarted Tor instance on port {new_port} for subnet {subnet}")
                return new_port
            else:
                logger.error(f"Failed to restart Tor instance for subnet {subnet}")
                return False
