...
class HAProxyManager:
    ...
    def send_command(self, command: str) -> str:
        try:
            ...
            if process.returncode == 0:
                ...
                logger.debug(f"HAProxy command '{command}' response: {response}")
                return response
            else:
    ...
    def add_server(self, backend: str, server_name: str, server_address: str) -> bool:
        try:
            ...
            if server_name in existing_servers:
                logger.info(f"Server {server_name} already exists in backend {backend}")
                return True
    ...
    def remove_server(self, backend: str, server_name: str) -> bool:
        try:
            ...
            if "Server deleted" in response or response == "":
                logger.info(f"Removed server {server_name} from backend {backend}")
                return True
            else:
                logger.error(f"Failed to remove server {server_name}: {response}")
                return False
    ...
    def add_backend_instance(self, instance_id: int, http_port: int) -> bool:
        try:
            ...
            logger.info(f"Adding backend instance: {server_name} -> {server_address}")
            
            success = self.add_server("privoxy", server_name, server_address)
            if success:
                logger.info(f"Successfully added backend instance {server_name}")
            else:
    ...
    def remove_backend_instance(self, instance_id: int) -> bool:
        try:
            ...
            if success:
                logger.info(f"Successfully removed backend instance {server_name}")
            else:
                logger.error(f"Failed to remove backend instance {server_name}")
    ...
    def update_servers_dynamic(self, backend: str, target_servers: Dict[str, str]) -> bool:
        try:
            ...
            logger.info(f"Current servers in {backend}: {list(current_servers.keys())}")
            logger.info(f"Target servers for {backend}: {list(target_servers.keys())}")
            ...
            logger.info(f"Updated servers in {backend}: {list(updated_servers.keys())}")
            ...
            for server_name, server_info in servers.items():
                if server_name.startswith("privoxy"):
                    try:
                        ...
                        logger.warning(f"Failed to parse server info for {server_name}: {e}")
    ...
    def create_config(self, instances: List[Dict]) -> bool:
        try:
            ...
            if not os.path.exists(self.socket_path):
                logger.error("HAProxy socket not available. Make sure HAProxy is running with socket enabled.")
                return False
            ...
            if instances:
                ...
                success = self.update_servers_dynamic("privoxy", target_servers)
                if success:
                    logger.info(f"Successfully configured HAProxy with {len(instances)} instances via Runtime API")
                else:
